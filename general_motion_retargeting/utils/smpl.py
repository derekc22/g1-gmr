...
def _scalar_to_float(value):
    """Coerce python ints, numpy scalars, or tensors to a float."""
    if isinstance(value, torch.Tensor):
        if value.numel() != 1:
            raise ValueError("Expected scalar tensor for mocap_frame_rate")
        value = value.detach().cpu().view(-1)[0]
    if isinstance(value, np.ndarray):
        if value.size != 1:
            raise ValueError("Expected scalar array for mocap_frame_rate")
...
def load_smplx_file(smplx_file, smplx_body_model_path):
    ...
    smplx_output = body_model(
        betas=torch.tensor(betas_array).float(), # (N, 16)
...
def get_smplx_data_offline_fast(smplx_data, body_model, smplx_output, tgt_fps=30):
    ...
    """
    src_fps = _scalar_to_float(smplx_data["mocap_frame_rate"])
